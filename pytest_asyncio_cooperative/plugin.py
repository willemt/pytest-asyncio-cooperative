...
async def do_teardown(teardown):
...
async def do_teardowns(item, teardowns):
    item.start_teardown = time.time()

    # We want to delay module/session scope fixtures
    pending_teardowns = []

    for teardown in teardowns:
        if getattr(teardown, "scope", "") in ["module", "session"]:
            pending_teardowns.append(teardown)
            continue
        await do_teardown(teardown)

    item.stop_teardown = time.time()

    return pending_teardowns
...
async def test_wrapper(item):
    ...
    try:
        ...
        item.stop = time.time()
        await do_teardowns(item, teardowns)
        # TODO: what to do with pending teardowns?
        raise

    item.stop = time.time()
    return await do_teardowns(item, teardowns)
...
async def run_tests(tasks, max_tasks: int, session, item_by_coro):
    ...
    while tasks:
        ...
        for result in done:
            ...
            # TODO: check session teardowns are OK
            # TODO: check module teardowns are OK
            # TODO: check class teardowns are OK???
            for teardown in all_teardowns:
                await do_teardown(teardown)
            ...
            item.runtest = wrap_in_sync(item)
